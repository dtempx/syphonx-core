...
- When asked to implement a task, look at [TASKS.md](TASKS.md) for the task definition; if not found, ask for clarification. Check the task off when done.
- Do not run the full test suite (`npm run test`) when implementing a new feature — it's slow. Do run any new tests generated by the task (e.g. `npx mocha --grep "category/N"`).
...
- **`schema.json`** — JSON Schema defining the template format (keep this update to date when making any API interface changes)
...
|----------|-------------|
| `attr` | Attribute extraction (`attr:href`, `attr:src`), format auto-defaulting |
| `blank` | Blank/missing value handling |
| `boolean` | Boolean type coercion |
| `cut` | `cut` query operator |
| `defaults` | Default value behavior |
| `distinct` | Duplicate removal |
| `each` | `each` action — iteration with `parent.index`/`parent.value` context |
| `errors` | `error` action — levels, stop behavior, conditional triggers |
| `extract` | `extract` (regex) query operator |
| `filter` | `filter` query operator |
| `format` | `format` option — `singleline`, `multiline`, `href`, `none` |
| `general` | General extraction and mixed scenarios |
| `hits` | Hit counting / metrics |
| `html` | `html` query operator, inner HTML extraction |
| `json` | `json` query operator — parse embedded JSON |
| `keypress` | `keypress` action (online) |
| `lib` | Internal library utilities |
| `not` | Negation / `negate` option |
| `number` | Numeric type coercion |
| `object` | Nested object extraction (`type:"object"`) |
| `offline` | Offline-mode specific behavior |
| `online` | Online/browser tests — click, scroll, waitfor, repeat+paginate |
| `params` | Template `params` passing and formula substitution |
| `pivot` | Pivot/transpose of repeated data |
| `remove-nulls` | Null filtering in repeated arrays |
| `repeat` | `repeat` action — loop with limit and break |
| `replace` | `replace` query operator |
| `required` | `required` option — error on missing result |
| `reverse` | Array reversal |
| `size` | Array size / `limit` option |
| `split` | `split` query operator |
| `switch` | `switch` action — first-matching case |
| `sync-offline` | Synchronous extraction (`extractSync`) — offline |
| `sync-online` | Synchronous extraction (`extractSync`) — online/IIFE |
| `text` | Text extraction, whitespace handling |
| `transform` | `transform` action — DOM mutation before/between selections |
| `union` | `union` option — merged alternative selectors |
| `unpatch` | State restoration after patching |
| `value` | `value` formula post-processing |
| `vars` | Template variables (`_`-prefixed fields → `vars`) |
| `when` | `when` conditional guards |
| `xpath` | XPath selector support |
...
> **Full API documentation** is in the [`docs/`](docs/) sub-directory: [`docs/README.md`](docs/README.md), [`docs/selectors.md`](docs/selectors.md), [`docs/dynamic-formulas.md`](docs/dynamic-formulas.md), [`docs/key-features.md`](docs/key-features.md), and [`docs/api/`](docs/api/) (generated TypeDoc). The sections below are a quick reference; the `docs/` files are comprehensive.
